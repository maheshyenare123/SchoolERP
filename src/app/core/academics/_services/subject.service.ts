...
export class SubjectService {
  ...
  createSubject(subject: SubjectDtoModel): Observable<SubjectDtoModel> {
    ...
    return this.http.post<SubjectDtoModel>(Constants.URL.HOST_URL+Constants.Academics.Class, subject, {headers: httpHeaders});
  }

  // READ
  getAllSubjects(): Observable<SubjectDtoModel[]> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    return this.http.get<SubjectDtoModel[]>(Constants.URL.HOST_URL+Constants.Academics.Class, {headers: httpHeaders});
  }

  getSubjectById(subjectId: number): Observable<SubjectDtoModel> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    return this.http.get<SubjectDtoModel>(Constants.URL.HOST_URL+Constants.Academics.Class+ `/${subjectId}`, {headers: httpHeaders});
  }
  ...
  updateSubject(subject: SubjectDtoModel): Observable<any> {
    ...
    return this.http.put(Constants.URL.HOST_URL+Constants.Academics.Class+'/'+subject.id, subject, {headers: httpHeader});
  }
  ...
  deleteSubject(subjectId: number): Observable<SubjectDtoModel> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = `${Constants.URL.HOST_URL+Constants.Academics.Class}/${subjectId}`;
    return this.http.delete<SubjectDtoModel>(url, {headers: httpHeaders});
  }

  deleteSubjects(ids: number[] = []): Observable<any> {
    const url = Constants.URL.HOST_URL+Constants.Academics.Class + '/deleteSubjects';
    const httpHeaders = this.httpUtils.getHTTPHeaders();
