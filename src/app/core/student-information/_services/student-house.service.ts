...
export class StudentHouseService {
  ...
  createStudentHouse(studentHouse: SchoolHousModel): Observable<SchoolHousModel> {
    ...
    return this.http.post<SchoolHousModel>(Constants.URL.HOST_URL+Constants.Front_Office.Student_House, studentHouse, {headers: httpHeaders});
  }

  // READ
  getAllStudentHouses(): Observable<SchoolHousModel[]> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    return this.http.get<SchoolHousModel[]>(Constants.URL.HOST_URL+Constants.Front_Office.Student_House, {headers: httpHeaders});
  }

  getStudentHouseById(studentHouseId: number): Observable<SchoolHousModel> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    return this.http.get<SchoolHousModel>(Constants.URL.HOST_URL+Constants.Front_Office.Student_House+ `/${studentHouseId}`, {headers: httpHeaders});
  }
  ...
  findStudentHouses(queryParams: QueryParamsModel): Observable<QueryResultsModel> {
    ...
    const url =Constants.URL.HOST_URL+Constants.Front_Office.Student_House + '/find';
  ...
  updateStudentHouse(studentHouse: SchoolHousModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    return this.http.put(Constants.URL.HOST_URL+Constants.Front_Office.Student_House+'/'+studentHouse.id, studentHouse, {headers: httpHeaders});
  }
  ...
  updateStatusForStudentHouse(studentHouses: SchoolHousModel[], status: number): Observable<any> {
    ...
    const url = Constants.URL.HOST_URL+Constants.Front_Office.Student_House+ '/updateStatus';
  ...
  deleteStudentHouse(studentHouseId: number): Observable<SchoolHousModel> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = `${Constants.URL.HOST_URL+Constants.Front_Office.Student_House}/${studentHouseId}`;
