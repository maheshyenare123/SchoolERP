...
import { ApproveRejectLeaveEditDialogComponent } from '../approve-reject-leave-edit/approve-reject-leave-edit.dialog.component';
...
export class ApproveLeaveListComponent implements OnInit {
  ...
  approveLeaveRequest(approveLeave: ApproveLeaveDtoModel) {
    //approve api
    debugger
    const _messageType = MessageType.Create;
    const dialogRef = this.dialog.open(ApproveRejectLeaveEditDialogComponent, { data: { approveLeave }, width: '450px' });
    dialogRef.afterClosed().subscribe(res => {
      if (!res) {
        return;
      }

      this.layoutUtilsService.showActionNotification("Approved Leave", _messageType);
  ...
  rejectLeave(approveLeave: ApproveLeaveDtoModel) {
    //disapprove api
debugger
    const _messageType = MessageType.Create;
    const dialogRef = this.dialog.open(ApproveRejectLeaveEditDialogComponent, { data: { approveLeave }, width: '450px' });
    dialogRef.afterClosed().subscribe(res => {
      ...
      this.layoutUtilsService.showActionNotification("Reject Leave", _messageType);
